...
def main(args):
    ...
    image_paths, labels = zip(*dataset_test.imgs)
...
#        interpolate_pos_embed(model, checkpoint_model)
    ...
    if args.eval:
        ...
        acc, f1, precision, recall, cls_report, auroc, all_pred = evaluate_test(dataset_test, model_best, device)
        ...
        with open('predictions_max_val.csv', mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Data Name', 'Probability', 'GroundTruth'])
            for i in range(len(all_pred)):
                image_path = image_paths[i]
                ground_truth = labels[i]
                probabilities = all_pred[i]
                writer.writerow([image_path, probabilities, ground_truth])
